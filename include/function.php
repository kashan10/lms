	...
	function use_code($code)
{
	...
	$expiry = $mydb->query("DELETE FROM invite_codes  WHERE expiry > GETDATE()");
    if ($expiry)
		return false;

	$result = $mydb->query("SELECT code FROM invite_codes WHERE code='$code'");
	if (!$result)
		return false;
	...
	$update = $mydb->query("DELETE FROM invite_codes  WHERE code='$code'");
	if (!$update)
		return false;
