...
class Student {
	...
	function find_all_students($name=""){
		...
		static function studentAuthentication($NIC,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblstudent` WHERE `NIC` = '". $NIC ."' and `STUDPASS` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysqli_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb->loadSingleResult();
		 	...
		 	$_SESSION['NIC'] 		    = $user_found->NIC; 
		 	$_SESSION['IMAGE'] 		    = $user_found->image;
		 	$_SESSION['CLASSNO'] 		= $user_found->Classno; 
		 	$_SESSION['TR'] 		    = $user_found->TR; 
		 	$_SESSION['CODE'] 		    = $user_found->Code;
		 	$_SESSION['YEAROFEXAM'] 	= $user_found->YearofExam; 
		   return true;
		 }else{
		 	return false;
		 }
    ...
    //$mydb->setQuery("SELECT GETDATE()");
	// $date = $mydb->loadSingleResult();
	  
	// $mydb->setQuery("DELETE FROM invite_codes  WHERE expiry > '$date'");
	 //$expiry = $mydb->executeQuery();
    //if ($expiry)
	//	return false;
	
    $mydb->setQuery("SELECT * FROM invite_codes WHERE code='$code'");
	...
	$mydb->setQuery("DELETE FROM invite_codes  WHERE code='$code'");	
